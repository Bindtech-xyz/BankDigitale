...
= User information REST API Extension
User information REST API Extension

The project **userInformationRestAPIExtension** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `groovy` language.
...
[source,bash]
----
git clone https://your.github.com/userInformationRestAPIExtension.git
cd userInformationRestAPIExtension/
mvn package
----

The build should produce a zip archive under the `target/` folder named `userInformationRestAPIExtension-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `userInformationRestAPIExtension-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
